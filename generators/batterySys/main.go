...
func main() {
	...
	global.Logger.Debug("BATTERY SYS GENERATOR IS ON")
	...
	global.Logger.Info("Battery system Server Started")
	if err := router().ListenAndServe(); err != nil && err != http.ErrServerClosed {
		global.Logger.Error("Battery system server failed", "error", err)
	}
